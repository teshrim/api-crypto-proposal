...
%
As such, we adopt the position that it is incumbent upon us to take an
\emph{API-centric} view of the primitives and protocols that we formalize.  In
the remainder of this section, we discuss specific areas to which we will apply
...
%
They are likely to want a simple, semantically-intuitive,
...
\end{align*}
%
The type and structure of the data object \varfont{Msg} would be separately
...
designed so that application data may be piggy-backed on the handshake. As such,
...
of the TLS $1.2$ record layer by PI Shrimtpon and coauthors~\cite{PRS11}. Boldyreva
...
compromising security.  \todo{Chris: can you expland upon this,
  please?  Why is it ``beautiful''?  Give an short example to
  illustrate it's use''}
...
are mandated.}\todo{Chris: is there actual text to add from your note?}
...
\begin{task}
  ...
  We will establish the syntax and security for DH-based UAKE whose API is
  similarly intuitive. \todo{Chris: This sentence doesn't make sense to
    me, in particular the ``whose API is similarly intuitive''. Can you reword it to be more clear?}
\end{task}
