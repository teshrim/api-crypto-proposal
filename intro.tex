...
For years, the cryptography research community has been emploring developers to ``please stop rolling your own crypto!''  An
...
subtlely, on parameter choices, padding schemes, protocol logic, etc. 
...
libraries, specifically the APIs they export, \emph{is hard
  too}. Moreover, fielded APIs have long-lifetimes, because changing them wrecks havoc upstream. 
%Changing APIs breaks the applications that call them, and
%fixing the applications costs real money.
%
So if a theoretical primitive it is hard to realize with existing libraries/APIs, then either it won't be
implemented, or (worse) it is prone to being implemented incorrectly.
...
theory that is less useful than we might thing, or even damaging.
%
...
implemented, i.e. syntax that is as ``API-like''.  Relatedly, security
notions should consider these functionalities, so as to have a more
practice-grounded view of the security of primtives.
...
that unpins modern cryptography. We think this is unfortunate, as
...
applies this interesting approach to entity authentication protocols, but we
...
The third point, that cryptographic primtives should aim to be
...
preemptively mitigate the damage resulting from common misusages of
...
nonce-based authenticated encryption schemes that did not forfeit all
...
(\todo{Chris: please do a search for ``misuse resistance'' and throw a
  few citations in here}), and as one of the diseratta in the CAESAR competition.
...
\todo{Chris: Summarize body section, focusing on tasks and (very briefly)
  prior work by me.}
\tsnote{Possible subsume implemenation-considerate formalisms into this?}
