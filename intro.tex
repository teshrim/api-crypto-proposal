...
likely to build secure applications to top of it~\cite{ABF+}. Our approach is
...
(PKE)~\cite{BBN+}.  The motivation for hedged PKE is quite practical: traditional
...
\paragraph{Tasks.}
We will...
\tsnote{Application developer APIs for secure send/receive of messges
...
assume are provided.} \tsnote{Secure-channel APIs for streaming data vs.
message-oriented data.  Look at ``data is a stream'' paper.} \tsnote{}
\tsnote{Authenticated key-exchange: examine existing protocols and APIs, distill
out design patterns/common features, abstract out the API that one wants as a
security engineer. } \tsnote{Close open questions from CrAPI paper}
