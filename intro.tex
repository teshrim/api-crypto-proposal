...
And the security guarantees that we prove can depend crucially on parameter
choices, padding schemes, protocol logic, etc., and in ways that may not be obvious to
non-experts. Our research community has developed a collective wisdom that helps
...
has heard less clearly.
%
To paraphrase various industry speakers at editions of Real World Crypto (e.g.,
David McGrew (Cisco), Daniel Kahn-Gilmore (ACLU), Thai Duong (Google)), and Paul Kocher in his invited lecture at Crypto'16:
%
Cryptographers need to understand that the design of cryptographic
software libraries, specifically the APIs they export, hard work and that,
once fielded, APIs are very slow to change.
%
\if{0}
  (Changes to APIs can break the applications built upon them, leading to
  unhappy customers, lost of revenue or reputation, etc. \cpnote{The most
  important point we can make here is that having to change applications using
  the API costs \emph{real} money. The other points you make are secondary.})
\fi
%
So no matter how good the theory is, if it is hard to implement with existing
code, it either won't be implemented, or it is prone to be implemented
incorrectly.

This proposal adopts the position that it is incumbent upon
academic cryptographers to hear this message, and to accept practical
...
%
Specifically, the tasks that we propose here start from the viewpoint that:
\begin{enumerate}
  \item theory is considerably more useful to \textit{real-world security}
    when it is mindful of how formalisms are translated into practice;
...
%In addition, we believe that ease-of-correct-implementation should be treated as a first-class
%    security property, not dismissed as a mere ``programming exercise''.

\noindent

    \cpnote{Rogaway-Stegers is about making it easier to design protocols around
    crypto. An extension is that if you're implementing the protocol, and all
    the crypto you need is available via APIs, then it should be easy to do
    right.}\tsnote{Use these words as seed for the rest of the intro.}
