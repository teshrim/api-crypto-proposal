...
\tsnote{Summarize Cryptographic API paper.}

\paragraph{API-driven Cryptography.}
The security community has published extensively on the \emph{usability} of APIs.  Intuitively, if an API is easy to use correctly, developers are more likely to build secure applications to top of it.  Thus by building usable APIs, one can hope to close security gaps between applications and the API.  Our approach is complementary, focusing on the relationships between primitives that APIs present (to application developers) and the associated primitives that crypto theory addresses.  We take the position that, in practice, APIs are primarily driven by non-security requirements (e.g. functionality, adoption, deployment).   As such, it is emcumbent upon us to we call our approach \emph{API-Centric Cryptography}, as w
