...
\item when theory gives formal syntax for a primitive, it should be clearly suggestive of an actual API that is aligned with what developers want \tsnote{not sure that this is the right wording, or even that it belongs in this list (seems a bit more down it the weeds than the others)}
...
\paragraph{Prior Work.} \tsnote{Rogaway's partially elided protocols paper} \tsnote{Need to do some digging outside of crypto here!}\tsnote{This thread has the biggest risk of failure.  But also has potential for large return, as it would change the way we think about security in the community}
