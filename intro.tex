...
assume are provided.}
  ...
  \item \emph{Secure channel.}
    ...
    channels.
    %
    \cpnote{My observation of their paper is that they have a complex syntax
    that is intended to capture this complexity. There might be a simpler
    approach related to~ \cite{rogaway2009authenticated}.}
  ...
  \item \emph{Authenticated key-exchange.}
    ...
    %
    \cpnote{There's also a technical barrier: TLS cannot be shown to have key
    indistinguishibility, since the last step of the handshake uses the
    negotiated key.}
    ...
    %
    \cpnote{The right model will be the ``Authentication without elision''
    paper. \cite{rogaway2009authentication}.}
...
\tsnote{Might want to be more concrete?  Or give a concrete example?}
...
\paragraph{Tasks.}
\begin{itemize}
  \item \emph{Follow-on work to hedging paper.} \todo{Chris}\cpnote{My idea is to
    reconsider Mihir and Tung's paper on fast, standard-model hedged PKE. The
    angle is to \emph{directly} model the source of randomness as a PRNG with
    inputs and see what sort of security we can achieve. This gets around the
    "touching the coins" problem, since many RNGs let you add entropy to their
    state even if the encryption API doesn't surface coins.}
