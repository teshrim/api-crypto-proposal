...
%Once published, the syntax for a given cryptographic primitive tends to retain much of its original shape over time.  Follow-on work typically adopts the syntax, and concerns itself with ``flashier'' tasks, such as proving tighter security bounds, finding more efficient realizations, or moving from an idealized model (e.g. the random-oracle model) to the standard model.  
