...
expanded version of this  plea might be: ``cryptography is hard, so let the
crypto experts handle the crypto; you just worry about implementing what we
provide.'' There is good sense in this message.  Cryptography \emph{is} hard.
...
\fi
%
So no matter how pretty your theory is, if it is hard to implement with existing
...
\begin{enumerate}
  \item crypto theory is more useful and impactful when it endeavors to respect
    the APIs that existing crypto libraries expose;
%
  \item crypto theory should be mindful of how
...
%    \cpnote{Agreed. We could say that this is one way to achieve item~2.}
  ...
  \item ease of correct implementation should be treated as a first-class
    ...
    %
  \tsnote{Look at Rogaway's Partially
    Specified Protocols/Authentication without Ellision paper}
    \cpnote{This paper is about making it easier to design protocols around
