...
%
\cpnote{You use the word ``customer'' above instead of ``consumer.''}
%
...
Indeed, the idea of \emph{syntax-as-API} is very useful for building intuition, and as
a pedagogical tool.
...
inputs.  In reality the decryption process needs to parse what is received from
the channel into the ciphertext~$C$ and any context information (aka associated
data) that was sent with~$C$.\tsnote{mention serialization?}  Moreover, the
